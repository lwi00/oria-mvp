    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <Card className="!p-3 text-center">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-accent-sport">
            Total
          </p>
        ...
        <Card className="!p-3 text-center">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-success-500">
            Goals
          </p>
          <p className="text-[20px] font-extrabold text-text-primary mt-1 tabular-nums leading-none">
            {goalMetWeeks}
      ...
      <Card className="!p-4 border-accent-sport/20">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`w-14 h-14 rounded-full flex items-center justify-center ${currentKm >= targetKm ? "bg-success-500/15" : "bg-accent-sport/10"}`}
          >
            {currentKm >= targetKm ? (
              <svg
                width="28"
        ...
        <div className="flex flex-col gap-0">
          {sorted.map((a, i) => {
            ...
            return (
              <div
                key={a.id}
                className={`flex items-center gap-3 py-3 ${i < sorted.length - 1 ? "border-b border-oria" : ""}`}
              >
                <div
                  className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${a.goalMet ? "bg-success-500/15" : "bg-oria-chip"}`}
                >
                  {a.goalMet ? (
                    <svg
                      width="16"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isThisWeek && (
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase bg-accent-sport/15 text-accent-sport">
                        Now
                      </span>
                    )}
                  </div>
          ...
          {sorted.length === 0 && (
            <p className="text-sm text-text-muted py-4 text-center">
              No activities yet — sync with Strava to import your runs.
            </p>
          )}
        </div>
      </Card>
