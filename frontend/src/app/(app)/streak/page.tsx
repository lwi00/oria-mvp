  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card className="!p-4">
        ...
        <p className="text-[12px] text-text-muted mt-2">
          {nextMilestone - count} more week{nextMilestone - count > 1 ? "s" : ""} to unlock the{" "}
          {nextMilestone}-week badge.
        </p>
      </Card>
      ...
      <Card className="!p-4">
          ...
          {MILESTONES.filter((m) => m <= 20).map((m) => {
            ...
            return (
              <div key={m} className="flex items-center gap-3">
                ...
                <span
                  className={`text-[13px] font-semibold tabular-nums w-16 text-right ${reached ? "text-accent-purple-bright" : "text-text-muted"}`}
                >
                  {apyAtM.toFixed(2)}%
                </span>
              </div>
        ...
        <p className="text-[10px] text-text-muted mt-3 leading-relaxed">
          Projected effective APY at each streak length, assuming the vault rate stays where it is
          today.
        </p>
      </Card>
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <p className="text-[22px] font-extrabold text-text-primary tabular-nums leading-none">
              {currentKm.toFixed(1)}
              <span className="text-[13px] text-text-muted font-medium"> / {targetKm} km</span>
            </p>
            <p className="text-[12px] text-text-muted mt-1">
              {pct >= 100
                ? "Goal reached!"
                : `${Math.max(0, targetKm - currentKm).toFixed(1)} km to go`}
            </p>
          </div>
          <div
            className={`w-14 h-14 rounded-full flex items-center justify-center ${pct >= 100 ? "bg-success-500/15" : "bg-accent-sport/10"}`}
          >
            {pct >= 100 ? (
              <svg
                width="28"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-[12px] text-text-muted mt-0.5">
                Get back to {longest} weeks for{" "}
                {(() => {
                  const f = formatMoney(5, user?.settings?.currency ?? "USD");
                  return `${f.symbol}${f.intPart}`;
                })()}
              </p>
            </div>
            <button
              onClick={() =>
                recoverStreak.mutate(undefined, {
          ...
          <div className="flex flex-col gap-2">
            {weekHistory.slice(-12).map((w, i) => (
              <div key={i} className="flex items-center gap-3 py-1.5">
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${w.met ? "bg-success-500/20" : "bg-oria-chip"}`}
                >
                  {w.met ? (
                    <svg
                      width="12"
