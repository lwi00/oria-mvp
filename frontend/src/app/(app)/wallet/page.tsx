...
export default function WalletPage() {
  ...
  const {
    ...
    refetch: refetchMorpho,
  } = useMorphoPositions(wallet?.walletAddr);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <section
        ...
        style={{
          background:
            "linear-gradient(160deg, rgba(139,92,246,0.10) 0%, rgba(255,255,255,0.03) 50%, rgba(245,158,11,0.06) 100%)",
      ...
      >
        ...
        <div className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-success-100/80 border border-success-500/20 backdrop-blur-sm">
          ...
          <span className="text-[13px] text-success-500 font-semibold tabular-nums">
            {t("wallet.earnedApy", {
              earned: `${earnedFmt.symbol}${earnedFmt.intPart}.${earnedFmt.decPart}`,
              apy: apy.toFixed(2),
            })}
          </span>
        </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-3 gap-2.5 mt-5">
          {(() => {
            ...
            return [
              {
                label: t("wallet.invested"),
                value: `${invFmt.symbol}${invFmt.intPart}.${invFmt.decPart}`,
                color: "text-text-primary",
              },
              {
                label: t("wallet.earnedLabel"),
                value: `${eFmt.symbol}${eFmt.intPart}.${eFmt.decPart}`,
                color: "text-success-500",
              },
              {
                label: t("wallet.yourApy"),
                value: `${apy.toFixed(2)}%`,
            ...
            <div
              key={s.label}
              className="text-center py-3 rounded-xl bg-oria-section border border-oria"
            >
              ...
              morpho.positions
                ...
                .map((p) => {
                  ...
                  return (
                    <div
                      key={p.vault.id}
                      className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-oria-section border border-oria"
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-text-muted mt-0.5">
                          {p.vault.curator} · {p.vault.chainName}{" "}
      ...
      {morpho && morpho.idle.some((c) => c.usdc > 0) && (
        <Card>
          <p className="text-sm font-bold text-text-primary mb-3 tracking-tight">
            {t("wallet.idleUsdc")}
          </p>
            ...
            {morpho.idle
              ...
              .map((c) => {
                ...
                return (
                  <div
                    key={c.chainKey}
                    className="flex justify-between py-2.5 border-b border-oria last:border-b-0"
                  >
                    <span className="text-sm text-text-secondary">
                      {t("wallet.usdcOn", { chain: c.chainName })}
                    </span>
          ...
          <p className="text-[10px] text-text-muted mt-3">{t("wallet.idleHint")}</p>
        </Card>
      )}
                  ...
                  {grouped[group].map((d) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <p
                          className={`text-[11px] font-medium ${d.status === "earning" ? "text-success-500" : "text-warning-500"}`}
                        >
          ...
          <span className="flex-shrink-0">
            {copied ? (
              ...
              <svg
                ...
                strokeLinecap="round"
                strokeLinejoin="round"
                className="group-hover:stroke-accent-purple-bright transition-colors"
              >
