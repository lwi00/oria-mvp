...
export default function ChallengeDetailPage() {
  ...
  const weeksLeft = Math.max(
    0,
    Math.ceil((new Date(c.endDate).getTime() - Date.now()) / (7 * 86400_000)),
  );
      ...
      <Card className="relative overflow-hidden !p-5 text-center">
        ...
        <div className="relative">
          ...
          <div className="flex justify-center gap-4 mt-4 text-[11px] text-text-muted">
            <span>
              <span className="text-text-primary font-bold tabular-nums">{c.members.length}</span>{" "}
              members
            </span>
            <span aria-hidden>·</span>
            <span>
              <span className="text-text-primary font-bold tabular-nums">
                {c.elapsedWeeks}/{weeksTotal}
              </span>{" "}
              weeks
            </span>
            <span aria-hidden>·</span>
            <span>
              <span className="text-text-primary font-bold tabular-nums">{weeksLeft}</span>w left
            </span>
          </div>
        </div>
      </Card>
      ...
      {!isMember && (
        ...
        >
          {joinChallenge.isPending
            ? "Joining…"
            : c.maxMembers && c.members.length >= c.maxMembers
              ? "Full"
              : "Join challenge"}
        </button>
      )}
            ...
            {c.milestones.map((m) => (
              <div key={m.key} className="flex items-start gap-3 relative">
                ...
                <div className="min-w-0 flex-1 pt-0.5">
                  <p
                    className={`text-[13px] font-semibold ${m.achieved ? "text-text-primary" : "text-text-secondary"}`}
                  >
                    {m.label}
                  </p>
                  {m.sub && (
        ...
        <div className="flex items-end gap-1 h-20 overflow-x-auto pb-1">
          {c.weeklyParticipation.map((w, i) => {
            ...
            return (
              <div
                key={w.weekStart}
                className="flex flex-col items-center gap-1 shrink-0 min-w-[26px]"
              >
        ...
        <div className="flex items-baseline justify-between mb-3">
          ...
          <div className="flex items-center gap-2 text-[10px] text-text-muted">
            <span className="inline-flex items-center gap-1">
              <span className="w-2.5 h-2.5 rounded-sm bg-gradient-to-br from-accent-purple to-accent-purple-bright" />{" "}
              met
            </span>
            <span className="inline-flex items-center gap-1">
        ...
        <div className="overflow-x-auto">
          <div className="inline-block min-w-full">
            ...
            <div className="flex gap-1 mb-1 pl-[112px]">
              {c.weeks.map((w) => (
                <div key={w} className="w-5 text-[8px] text-text-muted tabular-nums text-center">
                  {fmtWeek(w)}
                </div>
              ))}
            </div>
              ...
              {ranked.map((m) => {
                ...
                return (
                  <div key={m.id} className="flex items-center gap-1">
                    <div
                      className={`flex items-center gap-1.5 w-[108px] shrink-0 ${isMe ? "" : ""}`}
                    >
                      <Avatar
                      ...
                      />
                      <span
                        className={`text-[11px] truncate ${isMe ? "text-accent-purple-bright font-semibold" : "text-text-secondary"}`}
                      >
                        {m.user.displayName ?? "—"}
                        {isMe ? " (you)" : ""}
                      </span>
        ...
        <div className="flex flex-col">
          {ranked.map((m, idx) => {
            ...
            return (
              ...
              >
                <span className="text-[12px] font-bold text-text-muted tabular-nums w-5 text-center">
                  {idx + 1}
                </span>
                <Avatar
                  initials={getInitials(m.user.displayName)}
                ...
                <div className="min-w-0 flex-1">
                  <p
                    className={`text-[13px] font-semibold truncate ${isMe ? "text-accent-purple-bright" : "text-text-primary"}`}
                  >
                    {m.user.displayName ?? "—"}
                    {isMe ? " (you)" : ""}
                  </p>
                ...
                <span className="text-[12px] font-bold text-text-primary tabular-nums shrink-0">
                  {m.weeksMet}
                  <span className="text-text-muted font-medium">/{weeksTotal}</span>
                </span>
              </div>
            );
          })}
