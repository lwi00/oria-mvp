...
export default function ChallengesPage() {
  ...
  const myChallenges =
    challenges?.filter((c) => !!user && c.members.some((m) => m.userId === user.id)) ?? [];
  const otherChallenges =
    challenges?.filter((c) => !user || !c.members.some((m) => m.userId === user.id)) ?? [];

  const q = searchQuery.trim().toLowerCase();
  const filteredOtherChallenges = q
    ? otherChallenges.filter(
        (c) =>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 mb-5">
                {(["public", "friends"] as const).map((opt) => {
                  ...
                  const sub =
                    opt === "public" ? "Open to all Oria users" : "Visible to your friends only";
                  const Icon =
                    opt === "public" ? (
                      <svg
                        width="16"
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-7 h-7 rounded-lg flex items-center justify-center ${active ? "bg-accent-purple/25" : "bg-oria-chip"}`}
                        >
                          {Icon}
                        </span>
                        <span
                          className={`text-[13px] font-bold ${active ? "text-text-primary" : ""}`}
                        >
                          {label}
                        </span>
                      </div>
      ...
      {myChallenges.length > 0 && (
        <>
          ...
          {myChallenges.map((c) => {
            ...
            const myProgress =
              myMembership && myMembership.weeksTotal > 0
                ...
                <Card className="!p-4 !border-accent-purple/20 cursor-pointer hover:bg-oria-card-hover transition-colors">
                  <div className="flex justify-between items-start mb-2 gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-[15px] font-bold text-text-primary tracking-tight truncate">
                          {c.title}
                        </p>
                      ...
                      {user && c.creatorId === user.id && (
                        ...
                        >
                          <svg
                            ...
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            className="group-hover:stroke-error-500 transition-colors"
                          >
                  ...
                  {c.description && (
                    <p className="text-[12px] text-text-muted mb-2.5 line-clamp-2">
                      {c.description}
                    </p>
                  )}
                  ...
                  <div className="mb-2">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[11px] font-semibold text-accent-purple-bright tabular-nums">
                        {myMembership
                          ? `${myMembership.weeksMet}/${myMembership.weeksTotal} weeks`
                          : "0/0 weeks"}
                      </span>
                    </div>
        ...
        <>
          ...
          {filteredOtherChallenges.map((c) => {
            ...
            const avgProgress =
              c.members.length > 0
                ? c.members.reduce(
                    (sum, m) => sum + (m.weeksTotal > 0 ? m.weeksMet / m.weeksTotal : 0),
                    0,
                  ) / c.members.length
                : 0;
        ...
        <Card>
          <div className="text-center py-8">
            ...
            <p className="text-[12px] text-text-muted mb-3">
              Challenge your friends to run more and earn more together.
            </p>
            <button
              onClick={() => setShowCreate(true)}
