...
function suggestPlan(targetKm: number) {
  ...
  if (targetKm <= 15)
    return { sessionsPerWeek: 3, longRunKm: Math.round(targetKm * 0.4 * 10) / 10 };
  if (targetKm <= 30)
    return { sessionsPerWeek: 4, longRunKm: Math.round(targetKm * 0.35 * 10) / 10 };
  if (targetKm <= 50)
    return { sessionsPerWeek: 5, longRunKm: Math.round(targetKm * 0.3 * 10) / 10 };
    ...
    >
      ...
      >
        ...
        <p className="text-[13px] text-text-muted mb-5 leading-relaxed">
          Build the plan that fits you. Goal:{" "}
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between mb-2">
            ...
            >
              <div
                className={`relative w-[44px] h-[26px] rounded-full transition-colors ${hasLongRun ? "bg-accent-purple" : "bg-oria-strong"}`}
              >
                <div
                  className={`absolute top-[3px] w-[20px] h-[20px] rounded-full bg-white shadow-sm transition-transform ${hasLongRun ? "translate-x-[21px]" : "translate-x-[3px]"}`}
                />
              </div>
            </button>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-oria-section border border-oria mb-5">
          ...
          <div className="flex flex-col gap-2">
            ...
            {easyCount > 0 && (
              <div className="flex justify-between text-[13px]">
                <span className="text-text-secondary">
                  {easyCount} easy {sessionWord}
                  {easyCount > 1 ? "s" : ""}
                </span>
                <span className="font-semibold text-text-primary tabular-nums">
                  ~{perEasy.toFixed(1)} {unit} each
                </span>
              </div>
            )}
            <div className="border-t border-oria pt-2 mt-1 flex justify-between text-[13px]">
              ...
              <span className="font-bold text-accent-purple-bright tabular-nums">
                {(effectiveLong + perEasy * easyCount).toFixed(1)} {unit}
              </span>
            </div>
          </div>
        </div>
