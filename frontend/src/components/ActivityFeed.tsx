...
const TONE: Record<
  Tone,
  { border: string; bg: string; banner: string; text: string; chipIcon: string }
> = {
  gold: {
    border: "border-accent-gold/40",
    bg: "bg-accent-gold/[0.07]",
    banner: "bg-accent-gold/15",
    text: "text-accent-gold",
    chipIcon: "#F59E0B",
  },
  sport: {
    border: "border-accent-sport/40",
    bg: "bg-accent-sport/[0.07]",
    banner: "bg-accent-sport/15",
    text: "text-accent-sport",
    chipIcon: "#FC4C02",
  },
  purple: {
    border: "border-accent-purple/40",
    bg: "bg-accent-purple/[0.07]",
    banner: "bg-accent-purple/15",
    text: "text-accent-purple-bright",
    chipIcon: "#A78BFA",
  },
  green: {
    border: "border-success-500/40",
    bg: "bg-success-500/[0.07]",
    banner: "bg-success-500/15",
...
function milestoneKind(f: FeedEvent): MilestoneKind {
  ...
  if (p.firstStreakWeek === true || (f.eventType === "goal_met" && f.user.streakCount === 1))
    return "first_week";
  return null;
}
...
function milestoneIcon(kind: MilestoneKind, color: string) {
  switch (kind) {
    case "apy_tier":
      return <IconTrophy size={26} stroke={color} />;
    case "record":
      return <IconBolt size={26} fill={color} />;
    case "first_week":
      return <IconFlame size={26} fill={color} />;
    case "challenge":
      return <IconTrophy size={26} stroke={color} />;
    default:
      return <IconFlame size={26} fill={color} />;
...
function InteractionZone({ event, myId }: { event: FeedEvent; myId: string }) {
  ...
  const reactions: {
    key: "flame" | "trophy" | "bolt";
    node: React.ReactNode;
    activeClass: string;
  }[] = [
    {
      key: "flame",
      node: <IconFlame size={14} />,
      activeClass: "bg-accent-sport/15 border-accent-sport/40 text-accent-sport",
    },
    {
      key: "trophy",
      node: <IconTrophy size={14} stroke="currentColor" />,
      activeClass: "bg-accent-gold/15 border-accent-gold/40 text-accent-gold",
    },
    {
      key: "bolt",
      node: <IconBolt size={14} />,
      activeClass: "bg-accent-purple/15 border-accent-purple/40 text-accent-purple-bright",
    },
  ];
            ...
            return (
              <div key={c.id} className="flex items-start gap-2">
                <Avatar
                  ...
                  size={22}
                  colorSeed={c.authorId}
                  highlight={c.mine}
                />
                <div className="rounded-2xl rounded-tl-sm bg-oria-chip px-2.5 py-1.5 max-w-full">
                  <span
                    className="text-[11px] font-bold"
                    style={{ color: c.mine ? "#C4B5FD" : col.text }}
                  >
                    {c.author}
                  </span>
                  <span className="text-[12px] text-text-secondary ml-1.5 break-words">
                    {c.text}
                  </span>
                </div>
              </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1.5 rounded-full border text-[11px] font-semibold tabular-nums transition-colors cursor-pointer active:scale-95 ${
                active
                  ? r.activeClass
                  : "bg-oria-chip border-oria text-text-secondary hover:text-text-primary"
              }`}
              aria-pressed={active}
              aria-label={r.key}
            >
...
function StatChip({ icon, value, unit }: { icon: React.ReactNode; value: string; unit: string }) {
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-oria-chip border border-oria">
      ...
      <span className="text-[13px] font-extrabold text-text-primary tabular-nums leading-none">
        {value}
      </span>
  ...
  if (dist != null)
    chips.push(
      <StatChip
        ...
        value={dist.toFixed(dist % 1 === 0 ? 0 : 1)}
        unit="km"
      />,
    );
  const durMin =
    num(p, "durationMin") ??
    (num(p, "movingTimeSec") != null ? Math.round((num(p, "movingTimeSec") as number) / 60) : null);
  if (durMin != null)
    chips.push(
      <StatChip key="t" icon={<IconChatDots size={12} />} value={String(durMin)} unit="min" />,
    );
  if (f.user.streakCount > 0)
    chips.push(
      <StatChip
        key="s"
        icon={<IconFlame size={12} />}
        value={String(f.user.streakCount)}
        unit="wk streak"
      />,
    );
  return chips;
...
function MilestoneCard({ f, kind, myId }: { f: FeedEvent; kind: MilestoneKind; myId: string }) {
  ...
  const subtitle =
    kind === "apy_tier"
      ? `${f.user.streakCount}-week streak — bigger slice of the pool`
      : kind === "record"
        ? (() => {
            const rk = typeof p.recordKind === "string" ? p.recordKind : "distance";
            const d = num(p, "distanceKm");
            if (rk === "duration" && num(p, "durationMin") != null)
              return `${num(p, "durationMin")} min — your longest session yet`;
            if (rk === "speed" && num(p, "paceMinKm") != null)
              return `${num(p, "paceMinKm")} /km — your fastest pace yet`;
            return d != null ? `${d} km — your longest run yet` : "A new best — keep it rolling";
          })()
        : kind === "challenge"
          ? `"${typeof p.title === "string" ? p.title : "Challenge"}" — done and dusted`
        ...
        <p className="text-[12px] text-text-secondary flex-1 min-w-0 truncate">
          {isMine ? (
            ...
            <Link
              ...
              className="font-bold text-text-primary hover:text-accent-purple-bright transition-colors"
            >
              {name}
            </Link>
          )}
...
function ChallengeBlock({ userId, myId }: { userId: string; myId: string }) {
  ...
  const match = useMemo(() => {
    ...
    return (
      challenges.find((c) => c.status === "active" && c.members.some((m) => m.userId === userId)) ??
      null
    );
  }, [challenges, userId]);
    ...
    <div className="mt-3 rounded-xl border border-accent-purple/25 bg-accent-purple/[0.06] p-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[11px] font-semibold text-text-muted tabular-nums shrink-0">
          {weeksMet}/{weeksTotal} wks
        </span>
      </div>
      <div className="h-1.5 rounded-full bg-oria-chip overflow-hidden mb-2.5">
        <div
          className="h-full rounded-full bg-gradient-to-r from-accent-purple to-accent-purple-bright"
          style={{ width: `${pct}%` }}
        />
      </div>
      {iAmMember || joined ? (
    ...
    <div className="rounded-2xl border border-oria bg-oria-section p-3.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            {isMine ? (
              ...
              <Link
                href={`/friend/${f.userId}`}
                className="text-[13px] font-bold text-text-primary truncate hover:text-accent-purple-bright transition-colors"
              >
      ...
      <p className="text-[13px] text-text-secondary leading-snug mt-2.5">
        <span className="font-semibold text-text-primary">
          {isMine ? "You" : (f.user.displayName ?? "User")}
        </span>{" "}
        {text}
      </p>
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-[11px] text-text-muted mt-0.5">
            React, comment, keep the energy alive
          </p>
        </div>
        <Link
          href="/social"
          className="text-[12px] text-accent-purple-bright font-semibold hover:text-accent-purple"
        >
          Friends →
        </Link>
      </div>
          ...
          {events.map((f, i) => {
            ...
            return (
              <div key={f.id} className="flex flex-col gap-3">
                ...
                {kind ? (
                  <MilestoneCard f={f} kind={kind} myId={myId} />
                ) : (
                  <StandardCard f={f} myId={myId} />
                )}
              </div>
            );
          })}
