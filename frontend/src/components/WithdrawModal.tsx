...
export function WithdrawModal({ open, onClose }: Props) {
  ...
  const valid =
    numAmount > 0 && assets !== null && numAmount <= assets && shares !== null && shares > 0n;
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <p className="text-[13px] text-text-muted mb-4 leading-relaxed">{t("withdraw.intro")}</p>
            ...
            return (
              ...
              >
                ...
                <div className="text-right flex-shrink-0 ml-2">
                  <p
                    className={`text-[13px] font-bold tabular-nums ${hasPos ? "text-success-500" : "text-text-muted"}`}
                  >
        ...
        <div className="flex items-center justify-between mb-2 px-3 py-2.5 rounded-xl bg-oria-section border border-oria">
          <span className="text-[12px] text-text-muted">
            {t("withdraw.positionOn", { chain: vault.chainName })}
          </span>
          <button
